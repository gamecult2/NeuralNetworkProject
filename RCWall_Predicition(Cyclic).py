...
loaded_model = load_model("DNN_Models/DNN_LSTM-AE(CYCLIC)", custom_objects={'r_square': r_square})
info = 'LSTM-AE'
...
num_cycles = 6
repetition_cycles = 2
initial_displacement = 16
num_points = math.ceil(timeseries_length / (num_cycles * repetition_cycles))  # Ensure at least 500 points in total

parameters_input, max_displacement, name, sign = reftest()
displacement_input = generate_increasing_cyclic_loading_with_repetition(num_cycles=num_cycles, max_displacement=max_displacement, num_points=num_points, repetition_cycles=repetition_cycles).reshape(1, -1)[:, :500]
displacement_input = generate_increasing_cyclic_loading(num_cycles=num_cycles, initial_displacement=5, max_displacement=max_displacement, num_points=num_points, repetition_cycles=repetition_cycles).reshape(1, -1)[:, :500]
